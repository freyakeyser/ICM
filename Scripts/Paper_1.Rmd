...
dt.res <- NULL
n.dt.sims <- 20 # Number of simultions...
n.years <- 100
for(i in Stocks)
{
...
pop.dam.6 <- pop.dam %>% dplyr::filter(Species %in% c('morhua','harengus','aeglefinus','virens','solea','platessa'))
for.tunes.6 <- for.tunes %>% dplyr::filter(Species %in% c('morhua','harengus','aeglefinus','virens','solea','platessa'))
res.lambda.final.6 <- res.lambda.final %>% dplyr::filter(Species %in% c('morhua','harengus','aeglefinus','virens','solea','platessa'))
dub.time.6 <- dub.time.summary %>% dplyr::filter(Species %in% c('morhua','harengus','aeglefinus','virens','solea','platessa'))
# Lambda plot
...
res.lambda.final.6 <- res.lambda.final.6 %>% dplyr::group_by(Stock) %>% 
...
res.lambda.final.6$decade <- floor(res.lambda.final.6$year/10)*10

tst <- res.lambda.final.6 %>% dplyr::group_by(Stock,decade) %>% dplyr::summarise(mn.lam = mean(lam.fish,na.rm=T),
                                                                                 med.lam = median(lam.fish,na.rm=T),
                                                                                 sd.lam = sd(lam.fish,na.rm=T))
tst <- merge(tst,meta.sub,by="Stock")

tst <- tst %>% dplyr::arrange(Species,Area.broad) %>%
                                             dplyr::mutate(Stock.short = forcats::fct_inorder(factor(Stock.short, ordered=TRUE)))
              

lambda.dec.mn.plt <- ggplot(tst,aes(y=mn.lam,x=decade,group=Stock.short,color=Stock.short,shape = Stock.short,fill = Stock.short)) +
                            geom_jitter(size=4,width = 1) +
...
save_plot("D:/Github/ICM/Figures/tuned/lambda_decades_mean.png",lambda.dec.mn.plt,base_width = 30,base_height = 12)


lambda.dec.med.plt <- ggplot(tst,aes(y=med.lam,x=decade,group=Stock.short,color=Stock.short,shape = Stock.short,fill = Stock.short)) +
                            geom_jitter(size=4,width = 1) +
                            scale_x_continuous(breaks=seq(1940,2020,by=10))+
                            geom_hline(yintercept = 1,linetype='dashed') +
                            facet_wrap(~G.Species,scales='free_y') +
                            ...
                            guides(colour = guide_legend(ncol = 1))+
                            theme(axis.text.x = element_text(angle =45,hjust=1),legend.key.width = unit(5,'cm'))
save_plot("D:/Github/ICM/Figures/tuned/lambda_decades_median.png",lambda.dec.med.plt,base_width = 30,base_height = 12)


# Doubling Times
# Reorder the doubling time, either by Species and LME, or the other way around
dub.time.summary <- dub.time.summary %>%
                            dplyr::arrange(Species,LME) %>%
                            dplyr::mutate(Stock.short = forcats::fct_inorder(factor(Stock.short, ordered=TRUE)))
              
dub.heatmap.ne <- ggplot(dub.time.summary %>% dplyr::filter(LME == "NE Atlantic"), aes(x=year, y=Stock.short, fill= prop)) + xlab("") + ylab("")+                                    facet_wrap(~LME,nrow=3,scales='free_y',strip.position = "right") + 
...
dub.heatmap.nw <- ggplot(dub.time.summary %>% dplyr::filter(LME == "NW Atlantic"), aes(x=year, y=Stock.short, fill= prop)) + xlab("") + ylab("")+                                    facet_wrap(~LME,nrow=3,scales='free_y',strip.position = "right") + scale_x_continuous(labels=NULL) +
                            geom_tile() + scale_fill_viridis(name = "Proportion",discrete=FALSE) + 
                            theme(legend.position = 'none',text = element_text(size=12),plot.margin = margin(0,0.5,0,1.22, "cm"))

dub.heatmap.pac <- ggplot(dub.time.summary %>% dplyr::filter(LME == "Pacific (Alaska)"), aes(x=year, y=Stock.short, fill= prop)) + xlab("") + ylab("")+
...
dub.heatmap <- cowplot::plot_grid(dub.heatmap.pac,dub.heatmap.nw,dub.heatmap.ne,nrow = 3,rel_heights = c(3,4,10))
dub.heatmap


save_plot("D:/Github/ICM/Figures/tuned/Doubling_time_heatmap.png",dub.heatmap,base_width = 20,base_height = 17)

dub.time.6 <- dub.time.6 %>%
                            dplyr::arrange(LME) %>%
                            dplyr::mutate(Stock.no.species = forcats::fct_inorder(factor(Stock.no.species, ordered=TRUE)))
