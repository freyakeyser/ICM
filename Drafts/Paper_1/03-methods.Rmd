...
$$
Fish are expected to have variability in reproductive success by age (ref, ref). However, we had to make the simplifying assumption that each kilogram of spawning biomass contributed the same to the total total reproductive output, irrespective of age. We calculated the age-specific fecundity ($f_{y,a}$) as the number of offspring produced by each spawner in the population. The number of spawners in year $y$ and age class $a$ ($NS_{y,a]$) is the proportion of mature individuals in each age class ($Mat_{y,a}$) multiplied by the number of individuals in each age class:
...
<!-- This method does not account for any changes in reproductive success by age, each kilogram of spawning biomass contributes the same to the total total reproductive output, irrespective of their age. Finally, the age-specific fecundity ($f_{y,a}$) is calculated as number of offspring produced by each individual in the population.  -->
...
$$
The availability of catch-at-age time series in the data repository meant that fishing mortality and natural mortality rates were separable. Given this method provides separate estimates of $F$ and $M$ it was possible to calculate survival as well as theoretical population growth rates using the $M$ and fecundity ($\lambda_{F=0}$) estimates from the reconstruction. We recognize that the age structure in the underlying time series are still influenced by fishing pressure and that populations are at varying size. This means that ($\lambda_{F=0}$) does not represent the maximum intrinsic rate of abundance increase [e.g., r_max; CITE]. However, our theoretical annual growth rates in the absence of fishing represent the productivity that the population could have achieved in each specific year, if there had not been a fishery in that year. 

### Estimation

One limitation of calculating survivorship-to-age and fecundity-at-age using the archived data in the repository is that it is not straightforward to propagate observation error in each time series of data. To ensure that variability did not accumulate into unreasonable predictions for annual population productivity, we fit the Euler-Lotka model using an iterative minimization process, where predicted abundance in the following year:  
