...
if(load.results==F)
{
  ...
  {
    tmp <- rep(0,n.sim.yearss)
    print(paste0("Sim # ", s))

    for(y in 2:n.sim.yearss)
    {
      ...
      if(pop[y] >= 2*pop[1] & tmp[y] == 0) tmp[y:n.sim.yearss] <- 1
    } # end the years loop
    tmp.res[[s]] <- data.frame(year =1:n.sim.yearss,doubled = tmp,sim=s)
...
tmp <- plot(mod.dd.stock.gam.nf, pages = 0)
...
tmp <- plot(mod.dd.stock.gam.fish, pages = 0)
